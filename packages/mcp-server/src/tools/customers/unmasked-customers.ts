...
export const handler = async (client: Straddle, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.customers.unmasked(id, body)));
  } catch (error) {
    if (isJqError(error)) {
