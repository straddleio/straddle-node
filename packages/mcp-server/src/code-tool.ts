...
const prompt = `Runs JavaScript code to interact with the Straddle API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { STRADDLE_API_KEY: requireValue(readEnv('STRADDLE_API_KEY') ?? client.apiKey, 'set STRADDLE_API_KEY environment variable or provide apiKey client option'), STRADDLE_BASE_URL: readEnv('STRADDLE_BASE_URL') ?? readEnv('STRADDLE_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('STRADDLE_ENVIRONMENT') || undefined) as any },
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@straddlecom/straddle';
