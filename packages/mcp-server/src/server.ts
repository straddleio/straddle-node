...
async function getInstructions(stainlessApiKey: string | undefined): Promise<string> {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/straddle',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the straddle MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Straddle({
    ...{ environment: (readEnv('STRADDLE_ENVIRONMENT') || undefined) as any },
    logger,
