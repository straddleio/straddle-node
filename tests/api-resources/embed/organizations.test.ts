...
describe('resource organizations', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.embed.organizations.list({
    external_id: 'external_id',
    name: 'name',
    page_number: 0,
    page_size: 0,
  ...
  test('get: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.embed.organizations.get('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { 'correlation-id': 'correlation-id', 'request-id': 'request-id' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Straddle.NotFoundError);
  });
});
